...
pipeline {
    ...
    environment {
        DOCKER_IMAGE_NAME = "eribyteofficial/eriauth"
    }
    ...
    stages {
        stage("Checkout from github repo"){
            steps{
                ...
                git  branch: "${params.BRANCH}", url: 'https://github.com/EribyteVT/Eriauth'
            ...
            steps {
                ...
                withCredentials([string(credentialsId: 'CA_CERTIFICATE', variable: 'cert'),
                    ...
                    kubeconfig(caCertificate: "${cert}", credentialsId: "${cred}", serverUrl: "${url}"){
                        ...
                        sh "kubectl rollout restart deployment eriauth-$environ"
                    }
                }
            }
        }
